...
<p>Here they are. Read them and decide for yourself whether a CMO produced them:</p>

<ol>
<li><strong>Complete visibility and forecasting of AI workloads.</strong> Allocate every token, GPU-hour and managed-AI dollar to a team, a product and a feature. Forecast per workload. Alert on anomalies before the invoice does. This one is first because it is the multiplier &mdash; without allocation, every fix below is a one-off that regresses, because nobody owns the number.</li>
<li><strong>Model selection policies.</strong> Enforce which model tier each use case may call. Route classification, extraction and eval traffic to smaller models, enforced at the gateway, with evals so exceptions have to earn their cost. Frontier models are the default because nobody set a different default.</li>
<li><strong>Token efficiency.</strong> Prompt caching on repeated system prompts, context pruning, and batch endpoints for work that was never interactive in the first place.</li>
<li><strong>Bedrock commitment and capacity optimisation.</strong> Right-size provisioned throughput against p95 utilisation, move batch-tolerant jobs to batch inference, and use cross-region inference profiles to absorb burst instead of over-provisioning one region.</li>
<li><strong>GPU and inference infrastructure right-sizing.</strong> Reclaim idle GPU capacity, autoscale endpoints to observed demand, checkpoint training onto spot. Classic infrastructure hygiene &mdash; it just has not reached the AI stack yet.</li>
</ol>

<p>The rest of the campaign I could have done slowly and badly on my own: the topic, the story arc, the landing page, the copy, the branding. That list above is a different category. &ldquo;Right-size provisioned throughput against p95 utilisation&rdquo; is not a sentence a marketer writes from general knowledge. It needs somebody who genuinely knows how AI workloads get optimised &mdash; the kind of subject-matter expert whose calendar sets your launch date, because you book a week of calls and then wait on a review cycle. I do not have that knowledge. I have never had it. I now have work that carries it anyway, grounded in our own systems rather than in something a model remembered.</p>

<p>One thing I have left out, deliberately: each of those five carries a savings range in the campaign, and I am not publishing them here. They are directional industry ranges pending measurement against live data, and the campaign's own checklist says replace them before anything ships. Publishing them as fact in a post arguing for counted numbers would be the exact failure I describe further down.</p>

<p>And the campaign is not just those five. It is a recurring twenty-minute series with five episodes mapped, episode one's run of show timed minute by minute, landing page copy and the built page with the form slot marked, a four-email sequence, a blog post, and the slide designs I am building as I write this. Conventionally that is two to three weeks and five people minimum &mdash; ideate, write, design, build, and someone to run the schedule keeping the other four in sync &mdash; plus the SME whose calendar sets the actual timeline. There were no meetings. No planning session. No developer. No creative round. No waiting on an expert. One person, one session, and a set of agents with access to every system that holds our numbers.</p>
